...
사용자의 메시지 전체가 공백 제거 후 `>` 하나이면 직전 응답의 `다음 단계 진행해`와 동일하게 해석한다. 이 입력만으로 별도 승인이 필요한 위험 작업을 승인하지 않는다.
...
작업 유형 선택 기준은 `AGENTS.md ## 작업 유형 선택 규칙`이 정본이다.
Claude와 Codex가 같은 발화에서 같은 유형을 고르도록 공통 문서에 두고, 여기서는 중복 정의하지 않는다.
