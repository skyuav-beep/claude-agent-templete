...
- 사용자의 메시지 전체가 공백 제거 후 `>` 하나이면 직전 응답의 `다음 단계 진행해`와 동일하게 해석한다. 단, 별도 승인이 필요한 위험 작업을 새로 승인하는 신호로 사용하지 않는다. 상세 규칙은 `docs/approval-workflow.md`를 따른다.
...
같은 발화에서 두 런타임이 같은 작업 유형을 고르도록 하는 공통 판정 기준이다.
Claude Code는 `.claude/skills/`, Codex는 `.agents/skills/`가 이 기준을 따른다.

- 사용자 메시지에 명확한 유형 키워드(`기능`, `버그`, `리팩터링`, `리뷰`, `로직 변경`)가 있으면 해당 개별 유형을 우선 선택한다.
- 유형이 모호하거나 복합적일 때만 `request`로 분류한다.
- 동일 입력에서 두 유형이 동시에 매칭되면 더 구체적인 쪽을 선택한다.
- 각 유형의 트리거 키워드는 `.claude/skills/<name>/SKILL.md`와 `.agents/skills/<name>/SKILL.md`의 `description`이 정본이다.
- 위 규칙으로 유형이 정해지면 아래 구분 기준은 읽지 않는다.

### 유형이 갈릴 때만 적용하는 구분 기준

위 규칙으로도 두 유형이 계속 경합할 때만 적용한다. 유형이 이미 확정됐으면 이 절을 건너뛴다.
