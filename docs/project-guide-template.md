...
다음은 `templates/startup-checklist.md`, `templates/project-intake.md`, `templates/ui-intake.md`, `templates/tech-intake.md`, `templates/i18n-intake.md`의 "rider 운영 대시보드" 시나리오 답변을 위 1~12 섹션 구조에 그대로 옮긴 결과다. 실제 프로젝트에서는 이 형태의 `docs/project-guide.md`를 산출물로 만든다.

```
# Project Guide — Rider 운영 대시보드 MVP

## 1. Project Summary

- 한 줄 설명: 헬멧 세척 라이더 매칭 플랫폼의 본사 운영자 대시보드.
- 핵심 목표: 신규 라이더의 첫 주문 처리 시간을 24시간 → 1시간으로 단축.
- 주요 사용자: 본사 운영자(10명 이내), 매장 매니저(30개 매장).
- 이번 단계 범위: MVP 4주. 주문 목록/상세, 라이더 배차 현황, 수동 재배차, 일별 KPI 카드.

## 2. Confirmed Decisions

- 기능 범위: 주문 목록/상세, 라이더 배차 현황, 수동 재배차, 일별 KPI(주문 수 / 평균 배차 시간 / 미배차율) 3종.
- UI 방향: Linear/Stripe 참고. 정보 밀도 높은 운영 도구 톤. 미니멀, 라이트 테마 우선.
- 반응형 방향: PC 우선 (운영자는 PC, 매장 매니저는 태블릿까지). 모바일은 읽기 전용.
...
- 반드시(Must): 주문 목록/상세, 배차 현황 실시간 갱신, 수동 재배차, 일별 KPI 3종, 권한 3등급.
- 좋으면(Should): 주문 검색/필터, 라이더 상세 모달, KPI 기간 비교.
- 제외(Won't): 정산, 고객 CS, 라이더 모바일 앱, 다국어, AI 매칭 추천.

## 5. UI/UX Guidelines

- 톤: 정보 밀도 높음. 표/카드 중심. 색은 상태 신호용으로만 사용.
- 핵심 흐름: (1) 신규 주문 도착 → 5분 내 배차 확인, (2) 미배차 주문 수동 재배차, (3) 일별 KPI 확인.
- 필수 상태: loading, empty, error, partial-loaded(스트리밍), unauthorized.
- 접근성: 키보드 탐색 전체 가능, 색상 대비 WCAG AA, 토스트는 `aria-live=polite`.
- 콘텐츠 톤: 운영자 친화 한국어. 명령형보다 상태 서술형 ("배차 대기 3건").

## 6. Responsive Rules

- 모바일/PC 우선순위: PC 우선. 모바일은 읽기 전용 fallback.
- breakpoint: sm 640 / md 768 / lg 1024 / xl 1280. 작업 화면은 lg 이상 보장.
- 디바이스별 핵심 행동: PC는 단축키 + 다중 패널, 태블릿은 단일 패널, 모바일은 KPI/주문 요약만.
- 레이아웃 원칙: 좌측 글로벌 nav + 우측 컨텐츠. 컨텐츠 영역은 12열 그리드.
- 인터랙션 차이: 모달은 PC/태블릿 동일, 모바일은 풀스크린 시트로 전환.

## 7. Component Rules

- 분리 기준: 도메인 단위(`features/order`, `features/dispatch`) → UI primitives(`shared/ui`) → 인프라(`shared/lib`).
...
- 번역 범위: 미적용. 향후 매장 매니저 영문 지원 시 재논의.
...
- 수정 금지: 기존 라이더 앱 API 스키마, 라이더 인증 토큰 발급 로직.

## 10. Development Rules

- 구현 우선: 정확성 > 완성도 > 속도. 운영 데이터 다루므로 정확성이 최우선.
- 리뷰 우선: 권한 검증, 에러 처리, 빈 상태 처리.
- 테스트 기준: 비즈니스 로직(배차/권한)은 단위 테스트 필수, 핵심 흐름 3종은 e2e 필수.
