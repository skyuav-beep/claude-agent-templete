...
- STATE 리마인더 훅이 아무에게도 도달하지 않던 출력 규약 결함을 고쳤다. (2026-07-31)
  ...
  - 검증: bash 구문 검사, commit 명령 판별 3케이스 실증, 실제 세션에서 `additionalContext` 수신 확인(소비 프로젝트 signal2의 로컬 교체본 기준).
  - 전체 CI 대기열: 훅 스크립트 단독 변경으로 lint/test/build 대상 없음.

- 에이전트 가이드의 1단계 부트스트랩 규칙과 템플릿 원본 상태를 정합화했다. (2026-07-31)
  - `docs/approval-workflow.md` 1단계에서 이미 제공된 상위·저장소 가이드를 적용하고, 필수 응답 정책이 미로딩된 경우 `CLAUDE.md ## 커뮤니케이션`·`## 답변 포맷`만 읽을 수 있도록 예외를 명시했다. 그 외 저장소 읽기와 작업 분석 명령 금지는 유지했다.
  - `docs/project-guide.md`는 설치 manifest의 `project-owned` 초기 파일이므로 템플릿 배포 원본에서 scaffold 상태를 의도적으로 유지하고, 소비 프로젝트가 intake 결과로 교체한다는 경계를 명시했다.
  - 검증: diff 오류 0, manifest 배포·보호 정책 일치, 문서 인덱스 105개 최신, HTML 7개·인라인 스크립트 6개 통과.
...
- **`warn-design-tokens.sh`의 같은 출력 규약 결함 (2026-07-31 확인)**: 이 훅도 `exit 0` + stderr 경고라, 등록해도 경고가 모델에게 도달하지 않는다. 기본 미등록 opt-in이라 이번 수정 범위에서 제외했다. 켜려면 `state-reminder.sh`와 같은 방식(stdout JSON `additionalContext`)으로 함께 고쳐야 한다.
- **`ask` 결정의 무력화 조건 (2026-07-31 실증)**: `permissionDecision: "ask"`로 사용자 확인을 요청하는 훅은 편집이 자동 승인되는 권한 모드에서 프롬프트 없이 통과한다. 확실히 막아야 하는 항목은 `deny`나 `exit 2`를 써야 한다.
