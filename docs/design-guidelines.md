...
- 경고 전달 방식: stdout JSON의 `hookSpecificOutput.additionalContext`로 모델에게 전달한다. `exit 0`의 stderr는 Claude Code가 버려서 경고가 아무에게도 도달하지 않으므로 쓰지 않는다(`state-reminder.sh`와 같은 규약).
