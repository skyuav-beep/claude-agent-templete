...
- opt-in 훅 `warn-design-tokens.sh`의 입력 파싱 결함과 코드 인젝션 취약점 수정. (2026-07-28)
  - **결함 1(무력)**: 평면 JSON만 파싱해 현행 규약의 `tool_input` 중첩에서 파일 경로를 얻지 못했다. 등록해도 항상 통과 — 나머지 3종과 같은 상태였다. 구/신 비교로 확인(구버전 경고 0건, 신버전 정상 경고).
  - **결함 2(인젝션)**: payload를 `json.loads('''$INPUT''')` 형태로 **파이썬 소스에 문자열 삽입**했다. 파일 본문에 삼중따옴표가 들어가면 문자열이 조기 종료되고 이어지는 텍스트가 표현식으로 평가된다. 즉 **디자인 파일을 쓸 때마다 그 내용이 코드로 실행될 수 있었다.** `/tmp` 마커 파일 생성으로 실제 실행을 실증했고(구버전 실행됨·신버전 안전) 마커는 정리했다. opt-in이라 기본 미등록이었던 것이 결과적으로 노출을 막았다.
  - **수정**: 다른 3종과 동일하게 스크립트를 fd 3으로 주고 payload는 stdin 전용으로 받는다. `tool_input` 중첩 우선 + 평면·환경변수 폴백. 파싱을 2회에서 1회로 줄이고 첫 줄을 경로, 나머지를 본문으로 반환한다.
  - **추가**: MultiEdit의 `edits[].new_string`을 검사 대상에 포함했다. `docs/design-guidelines.md`의 활성화 안내가 이미 `Write|Edit|MultiEdit` matcher를 제시하고 있어 문서와 동작이 어긋나 있던 부분을 맞췄다.
  - **검증**: 14케이스 14/14 통과(현행 규약 인식 4·오탐/제외 3·항상 exit 0 3·인젝션 1·대용량 2·MultiEdit 1). 4MB 본문에서도 경고가 유지된다. 경고 전용·비차단 정책은 그대로다.
  - **미변경**: 기본 미등록 정책은 유지한다. 정적 검출의 오탐 우려는 그대로이므로 필요한 프로젝트만 명시 등록한다.
...
- 커밋·푸시 상태 점검 후 미푸시 커밋 원격 반영 및 세션 종료. (2026-07-28)
  ...
  - 템플릿 문서·규칙 변경은 없고 본 종료 기록만 추가했다.
