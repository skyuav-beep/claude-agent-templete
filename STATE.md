...
- 저장소 위생 정리 2건. (2026-08-22)
  ...
  - 검증: 저장소 마크다운 120개 링크 전수 검사에서 깨진 링크 0건, 마커 파일이 `git status`에 노출되지 않음을 확인했다.
...
- 현재 필수 대기 항목은 없다.
- 2026-08-22에 PR 3건이 누적됐다(#39·#40·#41). 모두 문서와 훅 스크립트 범위이며 bash 구문, 훅 동작 프로브(세션 조정 6종·배포 차단 25종), manifest 경로·중복, 링크 검사, HTML 검사로 각각 검증했다. 누적 기준(3~5건)에 닿았으므로 다음 세션 초반에 전체 로컬 CI를 한 번 돌릴지 판단한다.
...
- 가드레일 오탐이 재발하는지 본다. 현재 남은 한계는 두 가지다. 명령 문자열 안에 데이터로 들어 있는 `git commit`에 상태 리마인더가 반응하는 것, 그리고 파일 수정·stage·commit을 한 명령에 묶으면 훅이 도는 시점에는 아직 수정 전이라 감지하지 못하는 것이다. 둘 다 차단하지 않는 경고여서 실해는 없다. 병합 뒤 원격 브랜치를 지우는 `git push origin --delete`가 `git push --force` 패턴으로 오탐 차단되는 문제는 2026-08-21에 이어 08-22 세션에서도 두 번 재발했다. 실제 차단이라 6단계 cleanup이 끊기며, 이번에는 `gh api -X DELETE repos/<owner>/<repo>/git/refs/heads/<branch>`로 우회했다. 관찰 단계는 지났으므로 `block-destructive.sh`의 `git push` 패턴을 `--force`/`-f`에 한정하는 수정을 다음 작업으로 올린다. 08-22 세션에서 사용자에게 함께 고칠지 물었고 이번 범위에서는 제외했다.
...
- 세션 종료 시 상태 업데이트는 `git-cleanup` 스킬이 점검 항목으로 커버한다. 남은 것은 기록 문구 예시를 문서에 넣을지 판단하는 것뿐이다.
