...
- `fix/avatar-gradient-design-scope` 브랜치 처리. 커밋 1개로 프리뷰 화면 3종(`admin`/`user`/`user-mobile`)의 아바타 gradient를 시안 policy에 맞게 스코프 지정한 변경이다. PR이 없어 main에 반영되지 않았다.
