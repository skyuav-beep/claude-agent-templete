...
- `docs/user-fe-preview.html` 1행의 `dldjtj1` 제거. 한영 전환 없이 입력된 문자열이 HTML 최상단에 남아 브라우저로 열면 페이지 맨 위에 그대로 찍힌다. 2026-09-12 아바타 gradient 브랜치를 확인하다 발견했고 그 커밋과 무관한 별건이라 함께 고치지 않았다.
